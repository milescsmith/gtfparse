...
def _check_float_cov_and_FPKM(gtf_dict):
    for i, feature_name in enumerate(gtf_dict["feature"]):
        cov = gtf_dict["cov"][i]
        fpkm = gtf_dict["FPKM"][i]
        assert isinstance(cov, float), "Expected cov to be float but got %s : %s" % (
            cov,
            type(cov),
        )
        if feature_name == "exon":
            ...
            assert fpkm >= 0, "Expected non-negative FPKM for transcript, got %s" % (
                fpkm,
            )
